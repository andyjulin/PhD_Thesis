...
For our analysis, the particles analyzed in the detector include $\pi^\pm, K^\pm, \piO,$ and $\Ks$, and the decay modes used are shown in \Cref{tab:dtag_modes}.
...
More than one $D$ combination can be extracted from a given event, as long as it satisfies all other requirements (see \Cref{ssec:selection_cuts,ssec:tagging_cuts}).
