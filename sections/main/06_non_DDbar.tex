\chapter{Measurement of Hadronic Production and $\BFpsipptononDDbar$}
\label{ch:non_DDbar}
...
Each of these samples, and their integrated luminosities, can be seen in \Cref{tab:data_samples_non_DDbar}.
...
In addition to these datasets, the scan data described previously (see \Cref{ssec:data_samples}) is also used.
...
After applying these preliminary cuts, there are three groups of selection criteria for multihadronic events: Standard (SHAD), Loose (LHAD), and Tight (THAD).
...
These are shown in \Cref{fig:hadron_fits_3650_old} and the resulting signal amounts for each are listed in \Cref{tab:3650_old_results}.
...
The number of hadrons found in each data sample are determined from the fits shown in \Cref{fig:hadron_fits_3500_new,fig:hadron_fits_3542_new,fig:hadron_fits_3600_new,fig:hadron_fits_3650_new,fig:hadron_fits_3671_new}.
Reconstruction efficiencies are shown in \Cref{tab:3650_new_reconstruction} with the total results listed in \Cref{tab:3650_new_results}.
...
The results for each cut are shown in \Cref{fig:extrapolation_SHAD,fig:extrapolation_LHAD,fig:extrapolation_THAD}.

From these extrapolations, it is evident the highest energy new continuum point (\SI{3.665}{\GeV}) falls below the trend suggested by the other new continuum points and is inconsistent with the assumed linear behavior.
...
This behavior is discussed further in \Cref{sec:psip_background}, and BESIII plans to take considerably more data across this region in the near future.
...
From this new distribution, selections with SHAD, LHAD, and THAD are performed based off number of tracks selected by the simplified cut criteria shown in \Cref{tab:DDbar_corr_cuts} relative to the total.
...
Values for $\DO\aDO$ and $\Dp\Dm$ have been multiplied by the correction factors discussed in \Cref{ssec:DDbar_correction}.
...
Using the reconstruction efficiencies from \Cref{ssec:nonDDbar_rec_efficiency_psipp}, we can compute the contribution for each sample.
For the $\qqbar$ component, we scale the number of hadronic events found in the old continuum data based on \Cref{eq:eff_extrapolation} for both R1 and R2 separately.
...
The resulting numbers of hadronic events are shown in \Cref{tab:psipp_results}.
...
\subsection{Non-$\DDbar$ Branching Fraction Exploration ($\psip$ from Data)}
...
Additionally, the modification detailed in \Cref{sec:psip_background} provides an estimation of the actual value, but neither of these is sufficiently reliable to present a definitive BESIII result.
...
This will increase the extrapolated reconstruction efficiency fits, as seen in \Cref{fig:extrapolation_SHAD_none,fig:extrapolation_LHAD_none,fig:extrapolation_THAD_none}.
As a result, more hadrons are subtracted from the $\psipp$ data, and the corresponding branching fraction is lower, as shown in \Cref{tab:nonDDbar_xsec_psipp_none,tab:nonDDbar_bf_psipp_none}.
...
The cross sections for all background samples are also listed, where the $\DDbar$ values are taken from the results in \Cref{tab:xsec_rc_data_sys}.
