...
The selection process for such events is shown in \Cref{tab:beam_energy_selection_cuts}.
...
A comparison of the measured values to the online beam energies is shown in \Cref{fig:beam_energy_scan}.
...
The results of each procedure for the on-peak data are shown in \Cref{fig:beam_energy_psipp}.
...
This means the measured beam energies shown in \Cref{fig:beam_energy_scan} must be increased by \SI{2}{\MeV} to determine the true values.
The final results, along with the luminosities of each bin, are shown in \Cref{tab:luminosity}.
...
{The values shown are over the entire data sample, while calculations for the cross sections use the values of each energy point individually (see \Cref{tab:DTag_eff_E_bin}). The errors listed on the PDG branching fractions are shown for reference, and are not included in the efficiency errors.}
\label{tab:DTag_eff}
\end{table}
...
A mode-dependent change based on the PDG branching fraction errors (\Cref{tab:DTag_eff}) was applied to $\epsilon_m$ in \Cref{eq:DDbar_data}.
