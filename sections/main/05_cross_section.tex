...
Namely, it vanishes near \SI{3.81}{\GeV} then reappears for higher energies.
Indeed, this is also prominently seen in the splitting of signal events in $\mbc$, such as in \Cref{fig:fit_result_D0_20,fig:fit_result_D0_21,fig:fit_result_D0_22,fig:fit_result_D0_23,fig:fit_result_D0_24}.
In each of these, there is a peak from Born level events (near $m_{\DO} = \SI{1.865}{\GeV}$) and a peak from events affected by ISR (near the beam energy).
More over, the Born peak is steadily diminished over this range, but is again seen at higher energies, such as in \Cref{fig:fit_result_D0_30}.
This behavior is a strong indication for needing interference, as such a shape would be impossible to model with only two non-interfering Breit-Wigner components.
