...
The production rate for a pair of $D$ mesons coming from $\psipp$ at a given center-of-mass energy ($W$) can be calculated following an approach of Kuraev and Fadin \cite{ref:Kuraev:1985}:
\beq
\label{eq:xsec_rc}
\sigma^{\text{RC}}_{\DDbar}(W) = \int z_{\DDbar}(W \sqrt{1-x}) \, \sigma_{\DDbar}(W \sqrt{1-x}) \, \fancyF \, dx.
\eeq

\noindent 
This method includes Initial State Radiation (ISR), which effectively replaces the well-defined $\ee$ center-of-mass energy of the accelerator with a distribution of energies based on the fraction of radiated energy, $x$.

The Coulomb interaction between the charged mode  mesons ($\Dp \Dm$) is given by
\beq
\label{eq:z_Dp}
z_{\Dp \Dm}(W) = \frac{\pi \alpha / \beta_{\Dp}(W) }{ 1 - \text{exp} (-\pi \alpha / \beta_{\Dp}(W) ) } \times \theta (W - 2 m_{\Dp} ), \qquad \beta_D(W) = \sqrt{1 - \frac{ 4 m_D^2 }{ W^2 } },
\eeq

\noindent 
where $\beta_D$ is the velocity of the $D$ meson in the center-of-mass system.
The theta function is used to impose the step in the cross section at the production threshold.
For the neutral mode ($\DO \aDO$), there is no Coulomb interaction, meaning this factor only accounts for the $\DDbar$ energy threshold:
\beq
\label{eq:z_D0}
...
\beq
\label{eq:born_xsec}
\sigma_{\DDbar} = \frac{ \pi \alpha^2 }{ 3 W^2 } \beta_D^3(W) |F_D(W)|^2. 
\eeq
...
Each resonant piece is parametrized with a phase angle ($\phi$) relative to the non-resonant contribution:
\beq
\label{eq:form_factor}
F_D(W) = F_D^{\text{NR}}(W) + \sum_r F^{\text{R}_r}_D(W) \, e^{i \phi_r }.
\eeq
...
F^\text{R}_D(W) = \frac{ 6 \, W \sqrt{ (\Gamma_{ee} / \alpha^2 ) ( \Gamma_{\DDbar}(W) / \beta^3_D(W) ) } }{ M^2 - W^2 - i M \Gamma(W) },
\eeq
...
d_{\DDbar}(W) = \frac{ \rho_{\DDbar}^3(W) }{ \rho_{\DDbar}^2(W) + 1 }, \qquad \rho_{\DDbar} = q_D(W) \times R_0, \qquad q_D(W) = \sqrt{\frac{W^2}{4} - m_D^2}.%\left( \frac{ \beta_D \, W }{ 2 } \right) R_0.
\eeq

\noindent
Here, $q_D$ is the $D$ momentum in the center-of-mass frame, while $R_0$ represents the radius of the parent particle. 
The $\DDbar$ partial width listed in \Cref{eq:breit_wigner} is the total width rescaled according to $\BDD$, the sum of all $\DDbar$ decay modes of $\psipp$:
\beq
\label{eq:Gamma_DDbar}
\Gamma_{\DDbar}(W) = \Gamma(W) \times \BDD = \Gamma(W) \times (1 - \BnDD).
\eeq
...
Without a trustworthy measurement to use for our analysis, we elect to define a fit parameter as the product of the two, $\GeepsipptoDDbar = \Geepsipp \times \BDD$.
...
\label{eq:fancy_f}
\begin{align}
\fancyF &= \beta \, x^{\beta - 1} \left[ 1 + \frac{\alpha}{\pi} \left( \frac{\pi^2}{3} - \frac{1}{2} \right) + \frac{3}{4} \beta + \beta^2 \left( \frac{37}{96} - \frac{\pi^2}{12} - \frac{L}{72} \right) \right] = \beta \, x^{\beta - 1} F(W^2), \notag \\ 
& \qquad \qquad \qquad \beta = \frac{2 \alpha}{\pi} (L - 1), \qquad L = \log \left( \frac{W^2}{m_e^2} \right).
\end{align}

With this in mind, the integral in \Cref{eq:xsec_rc} can be simplified by taking advantage of the relatively constant values of $z_{\DDbar}$ and $\sigma_{\DDbar}$ over sufficiently small intervals.
As a result, we can split the full range of $x$ into narrow bins, and pull these two factors outside the integral in each one.
This leaves only $\fancyF$, which is simple to evaluate:
\beq
\int \fancyF ~dx \approx \sum\limits_{n=0}^N F(W^2) \int_{\tfrac{n}{N}}^{\tfrac{n+1}{N}} \beta x^{\beta-1}~dx = \sum\limits_n^N F(W^2) \left[ x_{\text{upper}}^\beta - x_{\text{lower}}^\beta \right],
\eeq
...
\noindent
The bracketed expression in \Cref{eq:x_terms} represents the maximum value of $x$ determined by the theta functions of \Cref{eq:z_Dp,eq:z_D0}.
To maintain sufficient precision with this interval approximation, the value of $N = 1024$ is used.
Combining this with the other factors in \Cref{eq:xsec_rc}, the cross section including the effect of ISR becomes
\beq
\label{eq:xsec_rc_simp}
\sigma^{\text{RC}}_{\DDbar}(W) = \sum_{n = 0}^N z_{\DDbar}(W') \, \sigma_{\DDbar}(W') \, F(W^2) \left[ 1 - \left( \frac{ 2 m_D}{ W } \right)^2 \right]^\beta \left[ \frac{ [(n + 1)^\beta - n^\beta] }{ N^\beta } \right],
\eeq
...
\beq
\label{eq:exp_model}
F_D^{\text{NR}} = \FNR \, \exp ( -q_D^2 / \aNR^2 ),
\eeq
...
It is evident the procedure using $\DDbar$ events provides more stability over the run range than using $\mumu$ events.
...
To correct our measurements for the scan data, we use the difference in average values between the two methods, $\DeltaE_{\DDbar} - \DeltaE_{\mumu} \approx \SI{1.86}{\MeV}$ (see \Cref{fig:on_peak_E_cm_fit}). 
Adding this difference to each scan data measurements, we obtain the center-of-mass values used in this analysis.
Comparing this to the initial scan data center-of-mass energies, which were higher than the measured values by \SI{1.29}{\MeV} (see \Cref{fig:scan_E_cm_fit}), we find the initial values of the scan data were notably low ($\SI{1.86}{\MeV} - \SI{1.29}{\MeV} \approx \SI{0.7}{\MeV}$).
...
{The tracking cuts are similar as for $\pipm / \Kpm$ tracks, but with the Barrel region ($|\cos\theta| > 0.86$) excluded.
There are also cuts on the deposited energy and the momentum of each track to ensure they are consistent with $\ee$ tracks.}
\label{tab:bhabha_cuts}
\end{table}

Applying these cuts to both data and MC identically, we use the resulting number of events found in the MC divided by the number of total generated events to determine the efficiency $(\epsilon_{\text{MC}})$.
...
\beq
\lum = \frac{N_{\text{Data}}}{\sigma_{\text{Bhabha}} ~ \epsilon_{\text{MC}}}
\eeq
...
\hline
\end{tabular}
\caption{Measured integrated luminosities for each energy bin.}
{The uncertainties listed are statistical errors from the data selection, as uncertainties from the MC statistics are negligible.}
...
{Each sample was generated using run-dependent accelerator conditions and center-of-mass energies.}
...
We measure the yields of both $\DO \aDO$ and $\Dp \Dm$ events with two-dimensional fits to $\DeltaE$ and $\mbc$, as defined in \Cref{ssec:dtag_reconstruction}.
...
{The effect provides a small correction to the efficiency of $\DO$ reconstruction.}
...
After applying the correction in \Cref{ssec:cp_correction}, the efficiency values (see \Cref{tab:DTag_eff_E_bin}) were combined with the luminosity (see \Cref{tab:luminosity}) and the signal values from each 2D fit (see \Cref{tab:xsec_rc_data}) to determine the cross section at each energy point.  Since each $\psipp$ produces a $\DDbar$ pair, a factor of 2 is included in the denominator to correct for double counting:
\beq
\label{eq:xsec_rc_data}
\sigma_{\DDbar}^{\text{RC}}(E_i) = \frac{ N_D(E_i) }{ 2 \, \epsilon_D(E_i) \, \mathcal{L}(E_i) }.
\eeq
...
\hline
$E_{\text{mid}}$ & \multicolumn{2}{c}{$N_{\DO \aDO}$} & \multicolumn{2}{c}{$\sigma^{\text{RC}}_{\DO \aDO}$ [\si{\nb}]} & \multicolumn{2}{c}{$N_{\Dp \Dm}$}  & \multicolumn{2}{c}{$\sigma^{\text{RC}}_{\Dp \Dm}$ [\si{\nb}]} \\
\hline
...
The uncertainties on the cross sections are statistical only and come from the signal fitting ($N_D$) and MC reconstruction efficiencies (see \Cref{tab:DTag_eff_E_bin}).
...
The uncertainties for each parameter are obtained by taking the difference between this result and the nominal fit (see \Cref{fig:vdm_results}).
...
\subsection*{$\pipm / \Kpm$ Particle Identification (PID)}
\label{ssec:sys_Kpi_pid}
A \SI{0.5}{\%} efficiency change \cite{ref:Rong:2015} was applied identically to the procedure for $\pipm / \Kpm$ Tracking.
As this is an overall scale change, the only variable significantly affected is $\GeepsipptoDDbar$.
...
A mode-dependent change equal to the PDG branching fraction uncertainties (see \Cref{tab:DTag_eff}) was applied to $\epsilon_m$ in \Cref{eq:DDbar_eff}.
As this is an overall scale change, the only variable significantly affected is $\GeepsipptoDDbar$.
...
The values are shown in \Cref{tab:systematics}.
...
$\Kpm / \pipm$ PID      & 0.000 & 0.020 & 1.297 & 0.025 \\
...
Most of the values for the $\psipp$ parameters are comparable to their statistical error, except for the mass, as seen in \Cref{tab:sys_stat_ratio}.
...
\hline  
($\sigma_{\text{sys}} / \sigma_{\text{stat}}$) & 3.000 & 1.088 & 1.381 & 1.229 \\
\hline
\end{tabular} 
\caption{A comparison of measured statistical and systematic uncertainties.}
...
\hline \\ [-10pt]
$E_{\text{mid}}$ [\si{\GeV}] & \multicolumn{3}{c}{$\sigma^{\text{RC}}_{\DO \aDO}$ [\si{\nb}]} & \multicolumn{3}{c}{$\sigma^{\text{RC}}_{\Dp \Dm}$ [\si{\nb}]} \\ [4pt]
\hline
3.7342 & 0.168 & 0.048 & 0.005 & \mcc{3}{-}            \\ 
3.7368 & 0.215 & 0.067 & 0.007 & \mcc{3}{-}            \\ 
...
We can also compare our results to the cross section values found in the previous analysis of the On-Peak $\psipp$ data sample of \SI{2.93}{\invfb} by Derrick Toth \cite{ref:Toth:2014}.
...
{The values measured previously using the On-Peak $\psipp$ data sample to reconstruct double-tag (DT) decays.  The first errors listed are statistical, while the second are systematic.}
